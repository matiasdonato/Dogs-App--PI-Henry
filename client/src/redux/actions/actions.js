export function getApiDogs() {
    return function(dispatch) {
        return fetch(`${REACT_APP_DOG_API}`)
            .then(r => r.json())
...
export function getDbDogs() {
    return function(dispatch) {
        return fetch(`${REACT_APP_DOG_SV_URL}/dogs`)
            .then(r => r.json())
...
export function searchDbDogs(breed) {
    ...
    return function(dispatch) {
        return fetch(`${REACT_APP_DOG_SV_URL}/dogs?name=${breed}`)
            .then(r => r.json())
...
export function getTemperaments() {
    return function(dispatch) {
        return fetch(`${REACT_APP_DOG_SV_URL}/temperaments`)
            .then(r => r.json())
...
export function getDogDetails(id) {
    if (id > 999) {
        return function(dispatch) {
            return fetch(`${REACT_APP_DOG_SV_URL}/dogs/${id}`)
                .then(r => r.json())
